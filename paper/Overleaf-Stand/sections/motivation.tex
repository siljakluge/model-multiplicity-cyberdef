...
\end{equation*}
Computing the exact value is expensive because it requires evaluating many feature coalitions. The implementation therefore uses a SHAP explainer with a background sample from the training data to approximate feature contributions without retraining a model for every coalition.
...
Explanation variability is measured over SHAP values computed for each model in $\mathcal{H}_{\epsilon}$. Let $\phi_{h,i,j,c}$ denote the SHAP value assigned by model $h$ to feature $j$ for sample $x_i$ and class $c$. The analysis records the range and variance across Rashomon-set models,
\begin{equation*}
    R_{i,j,c} =
...
Existing work has studied predictive multiplicity and explanation multiplicity in general ML settings~\cite{black2022model,marx2020predictive,brunet2022implications,laberge2023partial}, and cyber-security research has studied explainable intrusion detection and adversarial robustness. The intersection remains underexplored. In particular, there is limited evidence on how Rashomon-set disagreement and explanation variability interact for intrusion-detection data.
